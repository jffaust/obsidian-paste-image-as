...
export class ImageProcessor {
    ...
    async processImageBlob(blob: Blob) {
        try {
            const { targetFormat, jpegQuality, webpQuality } = this.plugin.settings;
            ...
            const { convertedBlob, width, height } = await this.convertBlob(blob, mimeType, quality);
    ...
    private convertBlob(blob: Blob, mimeType: string, quality: number): Promise<{ convertedBlob: Blob; width: number; height: number }> {
    ...
    private async saveImage(arrayBuffer: ArrayBuffer, extension: string): Promise<TFile | null> {
    ...
    private insertImageIntoView(file: TFile, originalWidth: number, originalHeight: number) {
